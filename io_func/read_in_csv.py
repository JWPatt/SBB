...
def csv_to_dict(file_name):
    data_portion = {}
    if os.path.isfile(file_name) and os.path.getsize(file_name) > 0:
        df = pd.read_csv(file_name, header=None, index_col=0, encoding='utf-8').reset_index().to_dict(orient='list')
...
def csv_to_empty_dict(file_name):
    data_portion = {}
    if os.path.isfile(file_name) and os.path.getsize(file_name) > 0:
        data_portion = set(pd.read_csv(file_name, header=None, encoding='utf-8')[0])
